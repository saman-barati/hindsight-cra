...
2.07 sits seven hundredths of a point above the top of the Low band. A sole trader in a cash-intensive sector, onboarded without ever being seen, expecting to take half of a five-figure monthly turnover in cash, is rated barely above the Bank's lowest band.
...
11.4 **Cash-intensive sole traders.** The worked example at 5.6 lands at 2.07, seven hundredths of a point above the top of the Low band, and my instinct says it should be High. Instinct is not evidence, which is why Step 4 tests the model against customers already known to have caused harm rather than against my intuition. If the back-test agrees with my instinct, the fix is more likely to be in the category weights at 4.1 than in the level definitions.
...
11.9a **What "requires approval" does and does not cover.** Added to answer the obvious objection to 11.9, that it is an excuse wearing governance as a costume. The distinction the Bank draws is this. A **calibration change** alters the documented policy - where a boundary sits, what an escalator covers, how the categories are weighted - and needs approval, because someone other than the model's author has to own a change in the firm's risk appetite. A **defect fix** makes the implementation do what the documented policy already says, and does not: leaving a control that is known not to fire as written is not governance. Four changes have therefore been applied without approval, all logged in `docs/04-model-validation.md` section 9. **Two are defect fixes.** Escalator 5.3(d) now fires on the whole of C1 level 5, having previously fired only on C2, which moves three customers to High. And C4 level 3 now covers the family and known close associates of any politically exposed person rather than only a domestic one, because regulation 35(1) and escalator 5.3(a) already covered those people and the library's wording left a foreign PEP's relative with no level to be recorded at; no rating changes on this population. **The other two are documentation fixes**, which change no ratings at all: escalator 5.3(b) has been reworded to describe the payment corridors the model has always tested, and to say "a further tax residence" because that is what factor G2 records. Those four are the only changes anywhere in this project that have been applied without approval. Everything else in this section is either a calibration change, an unfilled gap in the library, or a concern with no fix proposed yet, and none of it has been applied. 11.13 is the one entry that is partly both: the C4 wording defect in it was fixed, and the escalator change it recommends was not.
...
11.16 **Factor P1 conflates three things, and one common business account has no level.** P1 level 4 reads "business current account without cash handling or international payments" and level 5 reads "business current account with cash handling **and** international payments enabled". A business current account with cash handling but no international payments — the ordinary shape of a cash-intensive UK sole trader or small company — fits neither. It has to be recorded at level 4, whose wording then contradicts the file, which is exactly what happens in the worked example at 5.6: that customer expects half of a five-figure monthly turnover in cash, is recorded at P2 level 5, and sits at P1 level 4. Recording it at level 4 is defensible, because P2 and P3 record the cash and the international functionality separately and carry 5% of the model each — but it is defensible in spite of P1's wording rather than because of it, and P1 carries 7%, so the choice between its level 4 and its level 5 is worth 0.07 on the total. The fix is to write P1's levels about the product alone and let P2 and P3 carry the two functionalities, which changes which customers sit at level 5 and is therefore a calibration change under 8.1. This is the same class of gap as 11.10 and 11.12: a real customer with no level to sit at.
...
| Version | Date | Change |
|---|---|---|
| 0.9 | Sept 2026 | The distance of the worked example from the band boundary corrected at 5.6 and 11.4: 2.07 is seven hundredths above the top of Low, not one. 11.9a rewritten — it named the wrong two of the four applied changes as the defect fixes, and its list of what remains unapplied was wrong in both directions. 11.16 added, on the business account that factor P1 has no level for. Section 11 now runs to seventeen entries. |
